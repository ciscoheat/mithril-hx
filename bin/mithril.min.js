Mithril = m = new function app(window, undefined) {
	...
	m.render = function(root, cell, forceRecreation) {
		...
		if (m.redraw.strategy() == "current") cacheCurrentDOM(root, cell, id);
	...
	function cacheCurrentDOM(root, cell, id) {
		var cellType = Object.prototype.toString.call(cell);
		// If cell is an array, build from the root children, not the root itself.
		if(cellType == ARRAY) {
			// Comparison required here too?
			cellCache[id] = buildCacheFromDOM(root.childNodes);
		} else {
			var rootNode = buildCacheFromDOM(root);
			// Test if cell is the same as the rootNode
			if(compareNodes(rootNode, cell, true)) {
				cellCache[id] = rootNodes[i];
			} else {
				// Identify the current cell in the rootNodes
				for(var i = 0; i < rootNode.children.length; i++) {
					var child = rootNode.children[i];
					if(compareNodes(child, cell, true)) {
						cellCache[id] = child;
						break;
					}
				}
	...
	function buildCacheFromDOM(node) {
		var nodeClass = Object.prototype.toString.call(node);
		var output;

		if(node.nodeType == 3) { // TEXT_NODE
			output = new String(node.textContent);
			output.nodes = [node];
		} else if(nodeClass == "[object NodeList]") {
			output = [];
			output.nodes = [];

			for(var i = 0; i < node.length; i++) {
				var childNode = buildCacheFromDOM(node[i]);
				if(childNode) {
					output.nodes.push(node[i]);
					output.push(childNode);
				}
			}			
		} else if(node.nodeType != 8) { // not a COMMENT_NODE
			output = {
				attrs: {},
				children: [],
				nodes: [node],
			...
			for(var i = 0; i < node.attributes.length; i++) {
				output.attrs[node.attributes[i].name] = node.attributes[i].value;
			}

			for(var i = 0; i < node.childNodes.length; i++) {
				var childNode = buildCacheFromDOM(node.childNodes[i]);
				if(childNode) {
					output.children.push(childNode);
					output.children.nodes.push(node.childNodes[i]);
				}
			}
		}
	...
	function compareNodes(a, b, deepComparison) {
		if (a.tag != b.tag || 
			a.children.length != b.children.length ||
			Object.keys(a.attrs).join() != Object.keys(b.attrs).join()
		) {
			return false;
		}
		if(!deepComparison) return true;

		for(var i = 0; i < a.children.length; i++)
			if(!compareNodes(a.children[i], b.children[i], false)) return false;
	...
	m.module = function(root, module) {
		...
		if (!isPrevented) {
			var currentStrategy = m.redraw.strategy();
			if(currentStrategy != "current") m.redraw.strategy("all");
			m.startComputation();
			roots[index] = root;
